...
function Cn(x, y_ss, z, y_ls, ω) #(5) suivant le même découpage
    cn = 0.0;
...
function Cf(x, y_ss, z, y_ls, ω, v) #(6) suivant le même découpage
    ...
    for vbar in deleteat!(used_substations, findall(x->x==v,used_substations))
...
function loss(x, y_ss, z, y_ls) #(7)
    ...
    op_cost_nofail = 0.0;
    for ω in 1:Nws
        op_cost_fail   += sum( pw[ω] * cc(Cf(x, y_ss, z, y_ls, ω, v)) for v in 1:Vs);
        op_cost_nofail += pw[ω] * cc(Cn(x, y_ss, z, y_ls, ω));
    end
